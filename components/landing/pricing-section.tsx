...
const plans = [
  {
    name: "Free",
    description: "Explore Aevon with generous limits",
  ...
  {
    name: "Enterprise",
    description: "For teams and large deployments",
    price: { monthly: null, annual: null },
    features: [
      "Unlimited tokens",
      "All Aevon models",
      "Dedicated India infra",
      "Custom fine-tuning",
      ...
      <div className="max-w-[1400px] mx-auto px-6 lg:px-12">
        ...
        <div className="flex items-center gap-4 mb-12">
          <button
            ...
            className={`text-sm font-mono transition-colors ${!isAnnual ? "text-foreground" : "text-muted-foreground"}`}
          >
            Monthly
          </button>
          <button
            onClick={() => setIsAnnual(!isAnnual)}
            className={`relative w-12 h-6 rounded-full border transition-colors ${isAnnual ? "bg-foreground border-foreground" : "border-foreground/20"}`}
          >
            <span className={`absolute top-1 w-4 h-4 rounded-full bg-background transition-all ${isAnnual ? "left-7" : "left-1"}`} />
          </button>
          <button
            onClick={() => setIsAnnual(true)}
            className={`text-sm font-mono transition-colors ${isAnnual ? "text-foreground" : "text-muted-foreground"}`}
          >
            Annual <span className="text-[#eca8d6]">-20%</span>
          </button>
        </div>
